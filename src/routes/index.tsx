...
function ModelGraphic({ selection, wireframe, edges, onSelect }: { selection: Selection; wireframe: boolean; edges: boolean; onSelect: (value: Selection) => void }) {
  return <svg viewBox="0 0 700 500" className="h-full max-h-[620px] w-full max-w-[850px] drop-shadow-xl" role="img" aria-label="Mock three dimensional mounting bracket model">
    <g className={cn("model-shape", wireframe && "wireframe", !edges && "no-edges")}>
      ...
      <path onClick={() => onSelect("face")} className={cn("model-face", selection === "face" && "selected-part")} d="M236 243 365 312 482 246 365 181Z" />
...
function Tab({ value, label }: { value: string; label: string }) { return <TabsTrigger value={value} title={label} className="h-9 rounded-none border-b-2 border-transparent px-1.5 text-[9px] data-[state=active]:border-primary data-[state=active]:bg-transparent data-[state=active]:text-primary data-[state=active]:shadow-none">{label}</TabsTrigger>; }
